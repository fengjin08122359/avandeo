...
class gift_ctl_admin_gift extends desktop_controller{
    ...
    public function toAdd() {
        ...
        if( $aData['goods_type']=='gift' ) {
            ...
            if($o->getList('*',array('goods_id'=>$aData['goods_id']))) $is_edit = true;
        ...
        foreach($saveGift as $key=>$row){
            $flag = $o->save2save( $row );
        }
        #↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓记录管理员操作日志@lujy↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        if($obj_operatorlogs = kernel::service('operatorlog.sales')){
            if(method_exists($obj_operatorlogs,'giftsave')){
                $obj_operatorlogs->giftsave($aData['name'],$is_edit);
            }
        }
        #↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑记录管理员操作日志@lujy↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    ...
    function _prepareGoodsData( &$data ){
        ...
        foreach( $goods['product'] as &$row ) {
            ...
            if(isset($row['gift']['max_limit']) ||  !$row['gift']['max_limit'] ) $row['gift']['max_limit']=$row['store'];
