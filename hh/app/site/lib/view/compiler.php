...
class site_view_compiler
{
    ...
    function compile_main($attrs, &$compiler)
    {
        if ($attrs&&$attrs['view']){
			if($app_id = $attrs['app']){
				$cus_view = str_replace(array('"', '"'),array("", ""),$attrs['view']);
				$app_id = str_replace(array('"', '"'),array("", ""),$app_id);
				
				$compiler->controller->_vars['_MAIN_'] = $cus_view;
				return " echo  \$this->_fetch_compile_include(".$app_id.", '" . $compiler->controller->_vars['_MAIN_'] . "', array(), false); ";
			}
			else{
