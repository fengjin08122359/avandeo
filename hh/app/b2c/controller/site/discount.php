 ...
 * Time: 下午3:00
 */
class b2c_ctl_site_discount extends site_controller{
    public function glist($type_id=0,$orderBy=0,$page=1) {
        ...
        $this->set_tmpl('discount_list');
        $this->page('site/discount/list.html');
    }
    ...
    public function get_goods($filter,$page=1,$orderby){
        $goodsModel = app::get('b2c')->model('discount');
        ...
        foreach($goodsData as $k=>$v){
            $goodsData[$k]['discount_item'] = $this->get_discount_item($v['item_str'],$msg);
        }
        ...
        $this->pagedata['pager'] = array(
            ...
            'link' =>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_discount','act'=>'ajax_get_goods')),
        );
        return $goodsData;
    }
    ...
    private function screen($cat_id,$filter){
        ...
        $screen['props'] = $props;
        //排序
        $orderBy = $this->app->model('discount')->orderBy();
    ...
    public function ajax_get_goods(){
        ...
        if($goodsData){
            $this->pagedata['goodsData'] = $goodsData;
            $view = 'site/discount/type/grid.html';
    ...
    private function get_discount_props($arrProps,$aSpace){
        if( empty($arrProps) ){
            return null;
        }
        $goodsProps = array();
    ...
    private function get_discount_item($discount_item,&$msg){
        $mdl_goods = app::get('b2c')->model('goods');
        $mdl_products = app::get('b2c')->model('products');
        foreach($discount_item as $k=>$pro){
            ...
            $discount_item[$k]['name'] = $temp_goods[0]['name'];
            $discount_item[$k]['price'] = $temp_p['price']['price']['current_price'];
            $discount_item[$k]['image_default_id'] = $temp_goods[0]['image_default_id'];
        }
    ...
    private function get_crumb($type_id){
        ...
        $arr[] = array('link'=>$this->gen_url(array('app'=>'b2c','ctl'=>'site_discount','act'=>'glist')),'title'=>'折扣特卖');
        if($type_id){
            $type_info = app::get('b2c')->model('space_type')->getList('name',array('type_id'=>$type_id),0,1);
            $arr[] = array('link'=>$this->gen_url(array('app'=>'b2c','ctl'=>'site_discount','act'=>'glist','arg0'=>$type_id)),'title'=>$type_info[0]['name']);
        }
        return $arr;
    }
}