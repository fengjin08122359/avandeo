...
#        }
            ...
            switch($show_storage){
                ...
                case '3';//启用库存优化方案
                    if($goodsdata[0]['store_prompt']){
                        $store_prompt = app::get('b2c')->model('goods_store_prompt')->getList('*',array('prompt_id'=>$goodsdata[0]['store_prompt']));
                    }
                    if($store_prompt){
    ...
    function _get_product_price($productId,$aGoods,$member_lv){
        $goodsPrice = array();
        $productMemberPrice['price'] = $aGoods['product']['price'];
        $productsModel = $this->app->model("products");
        //市场价
    ...
    function _get_goods_promotion($goodsId,$aGoods,$member_lv_id){
        ...
        foreach($goodsPromotion['goods'] as $row){
            ...
            $productPromotion['goods'][$row['rule_id']]['gift'] = $gain_gift;
        }
        ...
        foreach($goodsPromotion['order'] as $row){
            ...
            $productPromotion['order'][$row['rule_id']]['gift'] = $gain_gift;
        }
        //赠品
        if($giftId){
            ...
            foreach($aGift as $key=>$row){
                ...
                $giftPromotion[$row['product_id']] = $aGift[$key];
            }
            $productPromotion['gift'] = $giftPromotion;
        }
