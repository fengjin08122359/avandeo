    ...
    function _info($id=0,$type='add'){
        ...
        $oGtype = &$this->app->model('goods_type');
            ...
            if($type == 'edit'){
				...
				$this->pagedata['cat']['cat_image_id'] = $aCat['cat_image_id'];
				$this->pagedata['cat']['tmpl_path'] = $aCat['tmpl_path'];
            }
        $this->pagedata['seo_info'] = $seoCat;
        $this->pagedata['gallery_setting'] = $gallery_setting;
		$this->pagedata['theme_url'] = kernel::get_themes_host_url().'/'.kernel::single('site_theme_base')->get_default();
        ...
        if($objCat->save($_POST['cat'])){
            ...
            if($obj_operatorlogs = kernel::service('operatorlog.goods')){
                if(method_exists($obj_operatorlogs,'goodscat_log')){
                    $obj_operatorlogs->goodscat_log($_POST['cat'],$olddata);
                }
            }
            if(kernel::single('b2c_search_goods')->is_search_status()){
                $associate = array(
                    'words' => $_POST['cat']['cat_name'],
                    'type_id'=> $_POST['cat']['cat_id'],
                    'from_type' => 'goods_cat',
                );
                app::get('search')->model('associate')->save($associate);
            }
            #↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑记录管理员操作日志@lujy↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
