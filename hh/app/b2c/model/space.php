...
class b2c_mdl_space extends dbeav_model{
    var $has_many = array(
        'space_item' => 'space_item',
        'images' => 'image_attach@image:contrast:space_id^target_id',
    );
    var $subSdf = array(
        'default' => array(
            'space_item'=>array(
    ...
    function _filter($filter){
        ...
        if($filter['priceto'] || $filter['pricefrom']){
            ...
            $temp_space_ids = array();
            foreach($list as $v){
                if($v['sales_price'] >= $pricefrom && $v['sales_price'] <= $priceto){
                    $temp_space_ids[] = $v['space_id'];
                }
            }
            if($temp_space_ids){
                $filter['space_id|in'] = array_merge((array)$filter['space_id|in'],$temp_space_ids);
                $filter['space_id|in'] = array_merge((array)$filter['space_id'],$filter['space_id|in']);
            }else{
                $filter['space_id'] = "-1";
            }
        }
