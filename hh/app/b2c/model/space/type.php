...
class b2c_mdl_space_type extends dbeav_model{
    var $has_many = array(
        'props' => 'space_type_props:contrast'
    );
    ...
    function dump2($filter,$field = '*'){
        ...
        $rs = $res[0];
        if($rs){
            $propsData = app::get('b2c')->model('space_type_props')->getList('*',array('type_id'=>$rs['type_id']));
            $props = array();
            ...
            if($props_ids){
                $props_value_data = app::get('b2c')->model('space_type_props_value')->getList('*',array('props_id'=>$props_ids));
            }
    ...
    function fetchSave($data,&$msg=''){
        ...
        if($this->db->selectrow('select * from sdb_b2c_space_type where name='.$this->db->quote($data['name']).'')){
