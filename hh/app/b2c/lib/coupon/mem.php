...
class b2c_coupon_mem {
    ...
    public function get_list_m($m_id=0,$nPage=null,$cpns_types=null) {
        if( empty($m_id) ) return false;
        $filter = array('member_id'=>$m_id);
        $filter['disabled'] = 'false';
        $filter['memc_isvalid'] = 'true';
        if(!is_null($cpns_types)){
        	$filter['cpns_types'] = $cpns_types;
        }
    ...
    public function get_list_m_red($m_id=0,$nPage=null) {
    	if( empty($m_id) ) return false;
    	$filter = array('member_id'=>$m_id);
    	$filter['disabled'] = 'false';
    	$filter['memc_isvalid'] = 'true';
