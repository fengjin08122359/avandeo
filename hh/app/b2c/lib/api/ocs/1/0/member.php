 ...
 * @author Rocky <dev@shopex.cn> <zhangjunhua@shopex.cn> <rocky.soar@gmail.com>
 * @date 2011-11-15
 */
...
class b2c_api_ocs_1_0_member {
    ...
    */
    ...
    public function add($request, $rpcService) {
        ...
        $tmp['pam_account']['psw_confirm'] = $request['password'];
        $tmp['contact']['email'] = $request['email'];
        if( ! $memberMdl->validate($tmp, $errMsg)){
            $rpcService->send_user_error('5001', app::get('b2c')->_('验证字段失败:'.$errMsg));
        }
        if ( ! $member_id = $memberMdl->create($tmp) ) {
            $rpcService->send_user_error('5002', app::get('b2c')->_('添加会员失败'));
        }
     ...
     * 会员登陆校验
     */
    public function username_verity( $request, $rpcService ){
        if( !$request['user_name'] ){
            $rpcService->send_user_error('5001', app::get('b2c')->_('重要参数缺失'));
        }

        $is_exists = false;
        if(app::get('pam')->model('account')->getList('account_id',array('account_type'=>'member','login_name'=>$request['user_name'])))
            $is_exists = true;
        else
            $is_exists = false;

        return array('available' => ($is_exists ? 'false' : 'true'));
    }
}
