 ...
 */
class b2c_order_service_discount implements b2c_order_service_interface
{
    ...
    public function get_order_object($arr_object=array(), &$order_items, $tml='member_order_detail')
    {
        ...
        //获取空间组合图片
        $discount_info = app::get('b2c')->model('discount')->getList('image_default_id',array('discount_id'=>$arr_object['goods_id']),0,1);
        $arr_object['thumb'] = $discount_info[0]['image_default_id'];
        ...
        $render->pagedata['discount_order'] = $arr_object;
        ...
        if( strpos($tml,'admin')!==false ) {
            $tpl = 'admin/order/discount/'.$tml.'.html';
        } else {
            $tpl = 'site/member/discount/'.$tml.'.html';
        }
