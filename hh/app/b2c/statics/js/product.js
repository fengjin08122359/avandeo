...
var AlbumZoom=new Class({
    ...
    initZoom: function() {
        if (this.loadingCont != null && !this.zoomsImg.complete && this.previewImg.width != 0 && this.previewImg.height != 0) {
            var p = this.container.getElement('.product-album-preview');
            this.loadingCont.setStyles({
                left: (p.getSize().x - this.loadingCont.getSize().x) / 2,
                top: (p.getSize().x - this.loadingCont.getSize().y) /2,
                visibility: 'visible'
            });
        }
        if (Browser.safari) {
        ...
        //计算大图位置
        var parent = this.preview.getParent('.product-album-preview');
        this.zoomsX = parent.getPosition().x + parent.getSize().x + 10;
