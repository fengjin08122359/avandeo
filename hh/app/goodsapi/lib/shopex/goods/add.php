    ...
    function shopex_goods_add (){
        ...
        if($json_images['images']){
            foreach($json_images['images'] as $arr_image){
                ...
                if($params['has_default_image'] && $params['default_image_path'] == $arr_image['source']){
                    $image_default_id = $image_id; //默认图片
                }
                $save_images[] = array(
                    'target_type' => 'goods',
                    'image_id' => $iamge_id
       ...
       if($params['prop_values']){
        ...
        if(empty($arr_products['products'])){
            ...
            $product_status = true;
            if($params['marketable'] == 'false')$product_status = false;
            ...
            $$arr_products['products'][0]['is_default'] = true;
        ...
        $goods_status = true;
        if($params['marketable'] == 'false')$goods_status = false;
        $save_data = array(
            'tag' => $save_tag,
        ...
        $default_product_id = $save_data['product'][0]['product_id'];
        $goods_url = kernel::single('base_component_request')->get_full_http_host().kernel::single('site_controller')->gen_url(array('app'=>'b2c','ctl'=>'site_product','arg0'=>$default_product_id));
