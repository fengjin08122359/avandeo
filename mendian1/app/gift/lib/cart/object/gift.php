 ...
 */
	...
	public function add_object($arr_data, &$msg='', $append=true)
	{
		$data = $arr_data;
            ...
            if(($flag=$this->_check($aSave,$msg))!==true) return $flag;
    ...
    public function _get($aData){
        ...
        foreach($aData as $_key => $row) {
            ...
            if($arr_gift[intval($params['gift_id'])]){
        ...
        return $aResult['cart'] ? $aResult : array();
    }
    /**
     *  获取购物车内所有赠品信息
     *
     ...
     */
    private function _check( &$data ,&$msg) {
            ...
            if($goods_id) {
                ...
                //其他判断
                    ...
                    if( $data['quantity']<=$arr_gift_info['store'] ) {
                        ...
                        if( $data['obj_ident'] == $v['obj_ident'] && $data['total_quantity']>$arr_gift_info['gift']['max_buy_store']){
                            $msg = $appgift->_('超出限购数量！');
                            return false;
                        }
    ...
    public function get_update_num( $data,$ident ) {
        ...
        foreach( (array)$data['cart'] as $row ) {
            if( $row['obj_ident']===$ident )
                return array(
                    'buy_price'=>'积分兑换',
                    'consume_score'=>-($row['consume_score'] * $row['quantity'])
                );
        }
    }
