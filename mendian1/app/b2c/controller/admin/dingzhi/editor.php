...
class b2c_ctl_admin_dingzhi_editor extends desktop_controller{
    var $simpleGoodsId = 1;
    ...
    function _editor($type_id){
        ...
        $this->pagedata['gtype'] = $objGtype->getList('*',array('type_type'=>'dingzhi'),0,-1);
...
//        $this->pagedata['uploader'] = $gimage->uploader();
         ...
         * add by qianzedong
         */
        $this->pagedata['style'] = app::get('b2c')->model('style')->getList('*');
        $this->pagedata['store'] = app::get('b2c')->model('store')->getList('*');
    ...
    function _prepareGoodsData( &$data ){
        ...
        if($tmpProduct && $goods['product'] && count($goods['product']) === 1){
            $goods['product'][0]['is_default'] = 'true';
        }
        return $goods;
    }
    ...
    function toAdd(){
        ...
        /**
         * add by qianzedong
         * 保存商品关联风格信息,保存商品关联加盟店信息
         * step1:删除这个商品的所有关联信息
         * step2:重新添加
         */
        $db = kernel::database();
        $delsql = "DELETE FROM `sdb_b2c_styleproduct` WHERE `goods_id`=".$goods['goods_id'];
        $db->exec($delsql);
        foreach ($goods['style'] as $key=>$value) {
            foreach ($goods['product'] as $val) {
                $tempaddsql = "INSERT INTO `sdb_b2c_styleproduct` (`style_id`,`goods_id`,`product_id`)VALUES(
                    '{$key}','{$goods['goods_id']}','{$val['product_id']}')";
                $db->exec($tempaddsql);
            }
        }

        $delsql = "DELETE FROM `sdb_b2c_storeproduct` WHERE `goods_id`=".$goods['goods_id'];
        $db->exec($delsql);
        foreach ($goods['storep'] as $key=>$value) {
            foreach ($goods['product'] as $val) {
                $tempaddsql = "INSERT INTO `sdb_b2c_storeproduct` (`store_id`,`goods_id`,`product_id`)VALUES(
                    '{$key}','{$goods['goods_id']}','{$val['product_id']}')";
                $db->exec($tempaddsql);
            }
        }
    ...
    function edit($goods_id){
        ...
        if(is_array($goods['product'])){
            foreach($goods['product'] as $k=>$v){
                ...
                if (!$goods['product'][$k]['freez']) $goods['product'][$k]['freez'] = '0';
         ...
         * add by qianzedong
         */
        $style_data = app::get('b2c')->model('styleproduct')->getList('*',array(
            'goods_id'=>$goods_id
        ));
        $styleproduct = array();
        foreach ($style_data as $key => $val) {
            $styleproduct[$val['style_id']] = $val['style_id'];
        }
        $this->pagedata['goods']['styleproduct'] = $styleproduct;

        $store_data = app::get('b2c')->model('storeproduct')->getList('*',array(
            'goods_id'=>$goods_id
        ));
        $storeproduct = array();
        foreach ($store_data as $key => $val) {
            $storeproduct[$val['store_id']] = $val['store_id'];
        }
        $this->pagedata['goods']['storeproduct'] = $storeproduct;
