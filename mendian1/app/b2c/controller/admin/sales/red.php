...
class b2c_ctl_admin_sales_red extends desktop_controller{
    ...
    function index() {
        ...
        $this->finder(
        		'b2c_mdl_coupons', array(
                'title'=>app::get('b2c')->_('红包'),
                'actions'=>array(
                                array('label'=>app::get('b2c')->_('添加红包'),'href'=>'index.php?app=b2c&ctl=admin_sales_red&act=add','target'=>'_blank'),
                            ),
        		'base_filter'=>$arrays,
    ...
    function _editor(){
        ...
        $this->singlepage('admin/sales/red/frame.html');
    }
    ...
    function _checkData($aData) {
        ...
        $obj_c_template = kernel::single($c_template);
    ...
    function download($cpnsId,$nums){
        ...
        if ($list = $mCoupon->downloadCoupon($cpnsId,$nums)) {
            $exporter->download(app::get('b2c')->_('红包代码'),'coupon',$nums, $list);
        }else{
            header("Content-type: text/html; charset=UTF-8");
            echo __('<script>alert("'.app::get('b2c')->_("当前红包未发布/时间未到,暂时不能下载").'")</script>');
        }
        //*/
    }

    function erwei(){
    	include(ROOT_DIR.'/phpqrcode/phpqrcode.php');
    	QRcode::png('http://store.galanz.com.cn/index.php/wap/member-acquire_coupon-'.$_GET['arg0'].'.html');
    	exit;
    }
