...
class b2c_ctl_wap_paycenter extends wap_frontpage{
    ...
    public function dopayment($pay_object='order',$combination_pay = false)
    {
        if ($pay_object)
        {
            ...
            if(!$orderMemberId[0]['member_id'] ||  $orderMemberId[0]['member_id'] != $arrMember['member_id'] ){
            ...
            if ($sdf['pay_app_id'] == 'deposit'){
                $member_info = app::get('b2c')->model('members')->getList('advance,advance_freeze',array('member_id'=>$arrMember['member_id']));
                $sdf['return_url'] = "";
            }else{
                if (!$sdf['return_url'])
