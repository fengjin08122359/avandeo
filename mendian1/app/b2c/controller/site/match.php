...
class b2c_ctl_site_match extends b2c_frontpage{
     ...
     * 产品列表
     */
    public function glist($cat_id='',$urlFilter=null,$orderBy=0,$page=1,$virtual_cat_id=null,$showtype=null) {
		...
		*/
        ...
        $this->set_tmpl('match_glist');
        $this->page('site/match/glist.html');
    }
    private function _add_pagedata(&$pagedata)
    {
        $style_status_data = kernel::single('b2c_style_data')->get_status();
        $pagedata['banner'] = kernel::database()->select("SELECT * FROM `sdb_b2c_style` WHERE (`status`&1)>0");
    }
    /**
     * 添加搜索条件
    ...
    public function ajax_get_goods(){
        ...
        if($goodsData){
            $this->pagedata['goodsData'] = $goodsData;
            $view = 'site/match/glist/type/'.$showtype.'.html';
    ...
    public function get_goods($filter,$page=1,$orderby){
        ...
        $styleData = &app::get('b2c')->model('style')->getList('*',array(),$pageLimit*($page-1),$pageLimit,' `style_id` DESC ',$total=false);
        $total = &app::get('b2c')->model('style')->count(array());
        ...
        $this->pagedata['pager'] = array(
            ...
            'link' =>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_match','act'=>'ajax_get_goods')),
        );
    ...
    public function diff(){
        ...
        $this->page('site/match/compare.html');
   }

    public function plist($style_id){
        return kernel::single('b2c_ctl_site_matchplist')->index($style_id);
    }
}