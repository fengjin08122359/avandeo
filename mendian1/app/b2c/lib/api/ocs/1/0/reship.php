    ...
    public function create(&$sdf, $thisObj)
    {
        ...
        else
        {
            ...
            if (!$cnt)
            {
                ...
                if($sdf['member_id']){
                    $memlist = app::get('pam')->model('account')->getList('account_id',array('login_name'=>$sdf['member_id']));
                    $sdf['member_id']= $memlist[0]['account_id'];
                }
                ...
                if ($arr_items)
                {
                    ...
                    if (!$has_error)
                    {
                        ...
                        //ajx  添加当同时联通了crm和ocs或erp时，ocs或erp退货需要触发退货同步到crm
