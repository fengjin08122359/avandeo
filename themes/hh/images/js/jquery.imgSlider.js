var img = {
	...
	play:function(i){
		...
		if(i>img.num){
			...
			if(parseInt(img.current.style.left)>510){
				//如果活动框到了最右边--完成
				//小图片向左
				...
				//活动框向右--完成
				img.animate = setInterval(function(){
					if(parseInt(img.current.style.left)<(current_left + img.imageNumWidth*(i-img.num)-img.imageNumWidth*(i-img.num)/5)){
						img.current.style.left = parseInt(img.current.style.left) + img.imageNumWidth*(i-img.num)/5 +"px" ;
