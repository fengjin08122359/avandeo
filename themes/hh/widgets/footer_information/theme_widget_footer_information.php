...
function theme_widget_footer_information(&$setting,&$smarty){
    ...
    footer_news_foo($lv, $iNodeId, $limit, $setting['showallart'], $oAN, $oMAI, $tmp['child'], $setting);
    $html = array();

    footer_news_show($smarty, $tmp['child'], $setting, $html, 0, $limit);
    if( !$setting['shownode'] ) {
...
function footer_news_foo($lv=1, $iNodeId=1, $limit, $showallart, $oAN, $oMAI, &$tmp, $setting) {
    if($lv<0)return;
    ...
    if(is_array($aNodes)) {
        foreach ($aNodes as $val) {
            ...
            footer_news_foo(($lv-1), $val['node_id'], $limit, $showallart, $oAN, $oMAI, $tmp['child'][$val['node_id']], $setting);
...
function footer_news_show(&$smarty, $tmp, $setting, &$html, $lv=0, &$limit) {
    if($setting['shownode'] && $lv!=0) {
        ...
        $html[] = footer_news_html($lv, $url, $tmp['info']['node_name'],$typeClass);
    ...
    if($tmp['article']) {
        ...
        foreach ($tmp['article'] as $idx=>$row) {
            ...
            if($limit>0){
                $html[$key] = footer_news_html($tmp_lv, $url, $article_title,'article-index');
            }
    ...
    if($tmp['child']) {
        foreach ($tmp['child'] as $row) {
            footer_news_show($smarty, $row, $setting, $html, $lv+1, $limit);
        }
    }
}

function footer_news_html($lv, $url, $name,$type) {
    return <<<EOF
<li>
