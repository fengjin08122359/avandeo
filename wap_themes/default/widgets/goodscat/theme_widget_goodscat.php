...
function getMap($depth = -1, $cat_id = 0) {
	...
	$rows = $cat_mdl->getList ( 'cat_name,cat_id,parent_id,is_leaf,cat_path,type_id,cat_image_id', array (), 0, - 1, 'p_order ASC' );
	...
	foreach ( $rows as $k => $row ) {
		if ($depth < 0 || substr_count ( $row ['cat_path'], ',' ) < $depth) {
			$cats [$row ['cat_id']] = array (
					...
					'cat_image_id'=>$row ['cat_image_id'],
					'title' => $row ['cat_name'],
					'link' => app::get ( 'wap' )->router ()->gen_url ( array (
							'app' => 'b2c',
							'ctl' => 'wap_gallery',
							'act' => 'index',
							'args' => array (
