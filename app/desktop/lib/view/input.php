...
class desktop_view_input{
    ...
    function input_image($params){
		...
		$_img_size = $params['image_size']?$params['image_size']:'s';
		if ($_img_size=='s'){
			$image_src = base_storager::image_path($input_value,$_img_size);
		}else{
			$image_src = base_storager::image_path($input_value);
		}
        ...
        $html = '<div class="image-input clearfix" style="width:'.$imageInputWidth.'px;'.(($params['_float'])?"float:".$params['_float']:"").'" gid="'.$domid.'">';
