...
class designer_ctl_admin_area extends desktop_controller{
	public function __construct($app) {
		parent::__construct($app);
		$this->_model = $this->app->model('area');
	...
	function index() {
		$custom_actions[] = array(
			...
			'icon'  => 'add.gif',
			//'disabled'=>'true',
			'href'   => 'index.php?app=designer&ctl=admin_area&act=add',
			'target' => 'dialog::{title:\''.app::get('b2c')->_('面积').'\',width:600,height:350}',
		);
		...
		$this->finder('designer_mdl_area', $actions_base);
	}
	function add(){
		$this->display('admin/area/add.html');
	}
	function save(){
