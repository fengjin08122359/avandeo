...
final class weixin_payment_plugin_wxpay extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    function callback(&$in){
        ...
        ksort($in);
        $unSignParaString = weixin_commonUtil::formatQueryParaMap($in, false);
        $checksign = weixin_commonUtil::verifySignature($unSignParaString, $in['sign'], $partnerKey);
...
//↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓公共函数部分↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
    ...
    private function get_cft_package($partnerKey){
        try {
            ...
            ksort($this->fields);
            $unSignParaString = weixin_commonUtil::formatQueryParaMap($this->fields, false);
            $paraString = weixin_commonUtil::formatQueryParaMap($this->fields, true);

            return $paraString . "&sign=" . weixin_commonUtil::sign($unSignParaString,weixin_commonUtil::trimString($partnerKey));
        } catch (Exception $e) {
            die($e->getMessage());
        }
    ...
    private function get_biz_sign($bizObj, $paySignKey) {
        ...
        try {
            ...
            ksort($bizParameters);
            $bizString = weixin_commonUtil::formatBizQueryParaMap($bizParameters, false);
