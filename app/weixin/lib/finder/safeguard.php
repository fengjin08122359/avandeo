...
class weixin_finder_safeguard{
    ...
    public function detail_basic($id){
        ...
        return $render->fetch('admin/shop/safeguard_detail.html');
    }
    ...
    function column_edit($row){
        if( $row['status'] != '1' ){
            ...
            $return = ' <a target="'.$target.'" href="index.php?app=weixin&ctl=admin_shop_safeguard&act=updatefeedback&_finder[finder_id]='.$_GET['_finder']['finder_id'].'&p[0]='.$row['id'].'">'.app::get('b2c')->_('接受维权').'</a>';
            return $return;
        }
    }
