    ...
    function _filter($filter,$tableAlias=null,$baseWhere=null){
        ...
        if($filter['login_account_local'] || $filter['login_account_email'] || $filter['login_account_mobile']){
            ...
            if($filter['login_account_mobile']){
                $aData = app::get('pam')->model('members')->getList('member_id',array('login_type'=>'model','login_account' => $filter['login_account_mobile']));
        ...
        if($store->store_id > 0){
            $filter .= " and (store_id = ".$store->store_id." or (store_id = 0 && substring_index(area,':',-1) in ('".implode("','",$store->share_area)."'))) ";
        }
        ...
        if($areaGroupId){
            ...
            if(is_array($area_ids_arr) && count($area_ids_arr) > 0){
                $filter .= " and substring_index(area,':',-1) in ('".implode("','",$area_ids_arr)."') ";
            }
        }
