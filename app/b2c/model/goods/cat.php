...
class b2c_mdl_goods_cat extends dbeav_model{
    ...
    public function getTree(){
        return $this->db->select('SELECT o.cat_name AS text,o.cat_id AS id,o.parent_id AS pid,o.p_order,o.cat_path,o.tmpl_path,o.cat_image_id,
        ...
        if($data){
            ...
            foreach($data as $i=>$value){
                ...
                $tmpCat[$value['pid']][] =array(
							...
							'tmpl_path'=>$data[$i]['tmpl_path']?$data[$i]['tmpl_path']:'',
							'cat_image_id'=>$data[$i]['cat_image_id']?$data[$i]['cat_image_id']:'',
                            'cls'=>$cls,
                            'url'=>$link
                        );
            }
    ...
    function cat2json($return=false){
        ...
        app::get('b2c')->setConf('kvstore_goods_cat_expires',time());
        if($return){
            return $contents;
        }else{
            return true;
        ...
        if( base_kvstore::instance('b2c_goods')->fetch('goods_cat.data', $contents) !== false ){
            ...
            if($result){
                ...
                foreach($result as $k=>$val){
					if (!$val['tmpl_path']){
						$result[$k]['url'] = app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_gallery','args'=>array($val['cat_id']) ));
					}else{
						$result[$k]['url'] = app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_cat','args'=>array($val['cat_id']) ));
					}
                }
