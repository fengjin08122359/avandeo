...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    function goodsParams($gid){
        ...
        $aGoods_list = $objGoods->getList("*",array('goods_id'=>$gid));
        //$params = $this->_get_goods_params($aGoods_list['0']['params']);
		$goodsObject = kernel::single('b2c_goods_object');
		$goodsBasic = $goodsObject->get_goods_basic($gid,$aGoods_list['0']);
		$goodsProps = $this->_get_goods_props($goodsBasic['type']['props'],$aGoods_list['0']);
        $productBasic['props'] = $goodsProps;//商品类型中扩展属性
		$this->pagedata['extends_params'] = $productBasic['props'];
    ...
    function albums($goodsid, $selected='def'){
        ...
        $shop['url']['placeholder'] = kernel::base_url().'/app/b2c/statics/images/imglazyload.gif';
