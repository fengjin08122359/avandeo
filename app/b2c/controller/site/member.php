...
class b2c_ctl_site_member extends b2c_frontpage{
    ...
    private function get_cpmenu(){
        ...
        $arr_bases = array(
            ...
            'member_account'=>array(
                ...
                'items'=>array(
                   ...
                   // array('label'=>app::get('b2c')->_('我的红包'),'app'=>'b2c','ctl'=>'site_member','link'=>'red'),
                ),
            ),
            array(
		...
		if ($arr_blance){
			$_cnt = count($arr_bases['member_account']['items']);
			$arr_bases['member_account']['items'][$_cnt] = $arr_blance;//预存款充值                    
		}
		if ($arr_recharge_blance){
			$_cnt = count($arr_bases['member_account']['items']);
			$arr_bases['member_account']['items'][$_cnt] = $arr_recharge_blance;//预存款充值 
		}
		if ($arr_point_coupon_exchange){
			$_cnt = count($arr_bases['member_account']['items']);
			$arr_bases['member_account']['items'][$_cnt] = $arr_point_coupon_exchange;//积分兑换优惠券
		}
    ...
    */
    ...
    function coupon($nPage=1) {
        ...
        $aData = $oCoupon->get_list_m($this->app->member_id,$nPage,0);
        if ($aData) {
        ...
        $total = $oCoupon->get_list_m($this->app->member_id,null,0);
    ...
    function red($nPage=1) {
    	$this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
    	$this->path[] = array('title'=>app::get('b2c')->_('我的红包'),'link'=>'#');
    	$GLOBALS['runtime']['path'] = $this->path;
    	$oCoupon = kernel::single('b2c_coupon_mem');
    	$aData = $oCoupon->get_list_m($this->app->member_id,$nPage,1);
    	if ($aData) {
    		foreach ($aData as $k => $item) {
    			if ($item['coupons_info']['cpns_status'] !=1) {
    				$aData[$k]['coupons_info']['cpns_status'] = false;
    				$aData[$k]['memc_status'] = app::get('b2c')->_('此种红包已取消');
    				continue;
    			}
    	...
    	$total = $oCoupon->get_list_m($this->app->member_id,null,1);
    	$this->pagination($nPage,ceil(count($total)/$this->pagesize),'coupon');
    	$this->pagedata['browser'] = $this->get_browser();
    	$this->pagedata['coupons'] = $aData;
    	$this->output();
	...
	public function acquire_coupon($_couponid=0){
		...
		if (!$_couponid){
// 			$this->splash('error',null,app::get('b2c')->_('优惠券信息不正确！'),true);
			$this->end(false,app::get('b2c')->_('优惠券信息不正确！'));
		}
		
		
		$_obj_member_coupon = $this->app->model('member_coupon');
		$_obj_coupon = $this->app->model('coupons');
		$_obj_rule = $this->app->model('sales_rule_order');
...
// 			$this->splash('error',null,app::get('b2c')->_('超过领取的次数限制！'),true);
			$this->end(false,app::get('b2c')->_('超过领取的次数限制！'));
		}
		
		### 生成优惠券 ###			
		$_coupon_info = $_obj_coupon->downloadCoupon($_couponid,1);
		
		$arr_m_c = array();
		$arr_m_c['cpns_id'] = $_couponid;
        $arr_m_c['member_id'] = $this->member['member_id'];
		...
		if($_arr_coupon[0]['cpns_types']=='1'){
			$this->end(true,app::get('b2c')->_('红包领取成功！'));
		}else{
			$this->end(true,app::get('b2c')->_('优惠券领取成功！'));
		}
		
		
// 		$this->splash('success',$_member_url,app::get('b2c')->_('优惠券领取成功！'),true);
	}
