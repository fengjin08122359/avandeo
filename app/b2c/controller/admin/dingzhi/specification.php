...
class b2c_ctl_admin_dingzhi_specification extends desktop_controller{
    ...
    function index(){
        ...
        $this->finder('b2c_mdl_specification',array(
            ...
            'actions' => array(
                array('label'=>'添加规格','icon'=>'add.gif','href'=>'index.php?app=b2c&ctl=admin_dingzhi_specification&act=add','target'=>'dialog::{title:\'添加规格\', width:800, height:420}'),
            ...
            'base_filter'=>array('type'=>'dingzhi'),
    ...
    function selSpecDialog($typeId = 0) {
        ...
        if($typeId){
            ...
            $oSpec = &$this->app->model('specification');
            $aSpec = $oSpec->getList('spec_id,spec_name,spec_memo',array('type'=>'dingzhi'),0,-1);
        }
