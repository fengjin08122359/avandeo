 ...
 * $ 2010-05-25 14:09 $
 */
class b2c_cart_object_discount implements b2c_interface_cart_object{
    ...
    {
        if(empty($arr_data) || empty($arr_data['discount']))
        {
    ...
    public function add_object($aData, &$msg='', $append=true,$is_fastbuy=false)
    {
        ...
        if(!$temp_arr = app::get('b2c')->model('discount')->getList('discount_id,name,item_str,marketable',array('discount_id'=>$discount['discount_id']),0,1)){
        ...
        $aSave = array(
            ...
            'params'       => array(
                'name'  =>  $temp_arr['name'],
                'discount_id'   => $temp_arr['discount_id'],
                'extends_params' => $extends_params,
            ),
            'quantity'     => $discount['num'],
        );

        if(kernel::single("b2c_cart_object_goods")->get_cart_status()) {
            $this->discount_object[$aSave['obj_ident']] = $aSave;
    ...
    public function delete($sIdent = null) {
        ...
        return $this->oCartObjects->delete(array('member_ident'=>$this->member_ident, 'obj_ident'=>$sIdent, 'obj_type'=>'discount'));
    }

    // 清空购物车中优惠券数据
    public function deleteAll() {
        return $this->oCartObjects->delete(array('member_ident'=>$this->member_ident, 'obj_type'=>'discount'));
    }

    // 统计购物车中组合数据
    public function count(&$aData) {
        // 购物车中不存在discount商品
        if(empty($aData['object']['discount'])) return false;
        $aResult = array(
            'subtotal_weight'=>0,
            'subtotal'=>0,
            'subtotal_price'=>0,
        ...
        foreach($aData['object']['discount'] as &$row) {
            $this->_count($row);
...
#echo "<pre>";var_export($aData['object']['discount']);exit;
        return $aResult;
    }
    ...
    private function _generateIdent($aData) {
        ...
        sort($product_ids);
        return "discount_".$aData['discount_id']."_".implode("-",$product_ids);
    }
    ...
    private function _get_basic(&$aData) {
        ...
        foreach($aData as $row) {
            ...
            if($extends_params = $row['params']['extends_params']){
                $discount_product_ids = array_keys($extends_params);
                $aProductId = array_merge($aProductId,$discount_product_ids);
            }
            $temp_discount_info = $mdl_discount->getRow('*',array('discount_id'=>$row['params']['discount_id']));
            $aResult[] = array(
                'obj_ident' => $row['obj_ident'],
                'obj_type' => 'discount',
                'discount_id'=>$temp_discount_info['discount_id'],
                'discount_name'=>$temp_discount_info['name'],
                'image_default_id'=>$temp_discount_info['image_default_id'],
                'discount'=>$temp_discount_info['discount'],
                'obj_items' => array(
    ...
    private function _count(&$aData) {
        ...
        //单个套餐的优惠金额
        $discount_discount = $this->omath->number_multiple(array($aData['subtotal'],(1-$aData['discount'])));
        //该套餐的总优惠金额
        $aData['discount_amount_prefilter'] = $this->omath->number_multiple(array($discount_discount,$aData['quantity']));
