...
class b2c_finder_coupons{
	...
	### 领取链接 ###
    public function column_aquired($row){
		$_render = app::get('site')->router();
		return $_render->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'acquire_coupon','arg0'=>$row['cpns_id']));
    }
    ...
    function detail_basic($id){
        ...
        $sql="select cpns_types from sdb_b2c_coupons  where cpns_id='$id'";
        $data=kernel :: database()->selectrow($sql);
        
        
        if($data['cpns_types']=='1'){
        	$render->pagedata['iddds'] = $id;
        }
