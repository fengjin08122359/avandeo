...
class b2c_finder_goods{
    ...
    function detail_basic($gid){
        ...
        $goods=$o->getList(
            ...
            rank_count'
            ,array('goods_id'=>$gid));
        $default_product = app::get('b2c')->model('products')->getList('product_id',array('goods_id'=>$gid,'is_default'=>'true'));
        if(!$default_product){
            $default_product = app::get('b2c')->model('products')->getList('product_id',array('goods_id'=>$gid));
        }
        $goods = current($goods);
