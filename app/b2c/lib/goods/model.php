...
//            $this->pagedata['SelectSpecValue'] = array('value'=>implode("、",(array)$specValue),'selected'=>0);
    ...
    function pre_process($data){
        ...
        foreach($product as $key=>$value){
            $spec_private_value_ids =$value['spec_desc']['spec_private_value_id'];
            foreach($spec_private_value_ids as  $val){
                $spec_private_value_id[] = $val;
            }
        }
        $spec_private_value_id = array_unique($spec_private_value_id);

        foreach($spec as $key=>$value){
            $options = array_keys($value['option']);
            foreach($options as $val){
                if(!in_array($val,$spec_private_value_id)){
                    unset($spec[$key]['option'][$val]);
                }
            }
