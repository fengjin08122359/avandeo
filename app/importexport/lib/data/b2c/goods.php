...
class importexport_data_b2c_goods { 
    ...
    public function dataToSdf($contents, &$msg){
        ...
        if( $goods['bn'] && $goodsRow = $goodsModel->getList('goods_id,spec_desc', array('bn'=>$goods['bn'])) ){
    ...
    public function _import_product_data($product_row,$goods_id,&$msg){
        ...
        foreach ( $product_row as $col=>$value){
            //会员价
            if( substr($col,0,15) == 'member_lv_price' ){
        ...
        $product['price'] = array(
            'price' =>array('price'=>$product_row['price']),
            'member_lv_price' =>$member_lv_price,
            'cost' =>array('price'=>$product_row['cost']),
            'mktprice'=>array('price'=>$product_row['mktprice']),
        );
